  ...
  : TestSuite{"rmcat-wired", UNIT}
{
    ...
    timeTC51.push_back (40);  bwTC51.push_back (2500 * (1u << 10)); // 2.5 Mbps
    timeTC51.push_back (60);  bwTC51.push_back (600 * (1u << 10)); // 600 Kbps
    ...
    tc51a->SetSimTime (simT);
    tc51a->SetBW (timeTC51, bwTC51, true); // FWD path

    RmcatWiredTestCase * tc51b = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.1-cbrlike"};
    tc51b->SetSimTime (simT);
    tc51b->SetBW (timeTC51, bwTC51, true); // FWD path
    tc51b->SetCodec (SYNCODEC_TYPE_PERFECT); // CBR-like traffic source

    RmcatWiredTestCase * tc51c = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.1-stats"};
    tc51c->SetSimTime (simT);
    tc51c->SetBW (timeTC51, bwTC51, true); // FWD path
    tc51c->SetCodec (SYNCODEC_TYPE_STATS); // statistical video source

    RmcatWiredTestCase * tc51d = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.1-trace"};
    tc51d->SetSimTime (simT);
    tc51d->SetBW (timeTC51, bwTC51, true); // FWD path
    tc51d->SetCodec (SYNCODEC_TYPE_TRACE); // trace-based video source

    RmcatWiredTestCase * tc51e = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.1-sharing"};
    tc51e->SetSimTime (simT);
    tc51e->SetBW (timeTC51, bwTC51, true); // FWD path
    tc51e->SetCodec (SYNCODEC_TYPE_SHARING); // content-sharing video source

    RmcatWiredTestCase * tc51f = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.1-hybrid"};
    tc51f->SetSimTime (simT);
    tc51f->SetBW (timeTC51, bwTC51, true); // FWD path
    tc51f->SetCodec (SYNCODEC_TYPE_HYBRID); // hybrid (trace/statistics) video source
    ...
    timeTC52.push_back (0);   bwTC52.push_back (4 * (1u << 20)); // 4 Mbps
    timeTC52.push_back (25);  bwTC52.push_back (2 * (1u << 20)); // 2 Mbps
    timeTC52.push_back (50);  bwTC52.push_back (3500 * (1u << 10)); // 3.5 Mbps
    timeTC52.push_back (75);  bwTC52.push_back (1 * (1u << 20)); // 1 Mbps
    timeTC52.push_back (100); bwTC52.push_back (2 * (1u << 20)); // 2 Mbps

    RmcatWiredTestCase * tc52 = new RmcatWiredTestCase{bw, pdel, qdel, "rmcat-test-case-5.2-fixfps"};
    tc52->SetSimTime (simT);
    ...
    timeTC53fwd.push_back (40); bwTC53fwd.push_back (500 * (1u << 10)); // 500 Kbps
    timeTC53fwd.push_back (60); bwTC53fwd.push_back (2 * (1u << 20)); // 2 Mbps

    timeTC53bwd.push_back (0);  bwTC53bwd.push_back (2 * (1u << 20)); // 2 Mbps
    timeTC53bwd.push_back (35); bwTC53bwd.push_back (800 * (1u << 10)); // 800 Kbps
    ...
    tc54->SetCapacity (3600 * (1u << 10));  // bottleneck capacity: 3.6 Mbps
    tc54->SetSimTime (simT);
    tc54->SetRMCATFlows (3, t0s, t0s, true);    // Forward path
    ...
    tstartTC55.push_back (1);  tstopTC55.push_back (299);
    tstartTC55.push_back (2);  tstopTC55.push_back (299);
    tstartTC55.push_back (3);  tstopTC55.push_back (299);
    tstartTC55.push_back (4);  tstopTC55.push_back (299);
    ...
    tstartTC56.push_back (60);  tstopTC56.push_back (240);
    ...
    tc56->SetSimTime (300);                 // Simulation time: 300s
    tc56->SetTCPLongFlows (1, tstartTC56, tstopTC56, true);    // Forward path
    ...
    tc57->SetRMCATFlows (2, t0s, t0s, true); // Forward path
    ...
    tc58->SetCapacity (3600 * (1u << 10));  // Bottleneck capacity: 3.6Mbps (same as TC5.4)
    tc58->SetSimTime (simT);
